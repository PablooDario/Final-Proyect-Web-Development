<?php 
  ...
  $c = 0;
  $sql = "SELECT * FROM `usuario` WHERE `Encuesta` = 1";
...
<head>
  ...
  <title>Profesores que contestaron la encuesta</title>
...
<body>
  ...
  <h1 style="font-size:60px; background-color:powderblue; text-align:center; border:2px solid #0756ff;">Docentes que contestaron encuesta</h1>
  <table class="content-table" style="width: 100%; overflow: scroll;">
    <thead>
      <tr>
        <th>Profesor</th>
        <th>Correo</th>
    ...
    <?php for($i=0;$i<$c;$i++): ?>
      <tr>
        ...
        <td><?= $correo[$i] ?></td>
        <td><a href="./../connect/pdfs/<?= $id[$i] ?>.pdf" class="btn red" id="corr" target="_blank" >Comprobante</a></td>
        <?php if($reseteo[$i]): ?>
          <td><button class="btn blue" id="reseteoencuesta" value="<?= $id[$i] ?>">Si</button></td>
        <?php else: ?>
          <td>NO</td>
        <?php endif; ?>
      </tr>
  ...
  <footer class="page-footer blue" style="position: fixed; bottom: 0; width: 100%;">
    <div class="footer-copyright">
      <div class="container">
      © 2023 Copyright
      <a class="grey-text text-lighten-4 right" href="https://www.escom.ipn.mx">Inicio</a>
      </div>
    </div>
  </footer>
  <script src="./js/general.js"></script>
  <script src="./js/reseteo.js"></script>
</body>
</html>